...
**Analítica separada por `tipo`**: `construirFiltro` admite `tipo`; las funciones de ventas (`ventas_top`/`clientes_top`/`totales_facturado`/`resumen-ventas.md`) fijan `tipo='venta'` por defecto y las de compras (`compras_top` = `ventasTop` con tipo compra, `proveedoresTop` = ranking por `emisor`, `totales_compras`, `resumen-compras.md`) `tipo='compra'`. Ambos `.md` viven en `/facturas` y se regeneran juntos (`regenerarResumenesFacturasSerie`). Las `desconocido` quedan fuera de las dos.
