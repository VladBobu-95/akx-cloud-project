...
import { z } from "zod";
import { chatear, tablaAMarkdown } from "../services/chat.service";
...
export const ctrlChatN8n = async (
  ...
  try {
    ...
    // Telegram no pinta la tabla aparte: se añade al texto en markdown.
    const tablaMd = resultado.tabla ? tablaAMarkdown(resultado.tabla) : "";
    res.json({
      ...resultado,
      respuesta: tablaMd ? `${resultado.respuesta}

${tablaMd}` : resultado.respuesta,
